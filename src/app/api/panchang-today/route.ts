...
export async function POST(request: NextRequest) {
  ...
  if (!apiKey) {
    return NextResponse.json(
      { error: "Set FREEASTRO_API_KEY in .env.local before loading Panchang." },
      { status: 500 },
    );
  }

  const rawBody: unknown = await request.json().catch(() => null);
  const parsed = parsePanchangInput(rawBody);
  if ("error" in parsed) {
    return NextResponse.json({ error: parsed.error }, { status: 400 });
  }

  const { date, time, ...location } = parsed.input;
  const [year, month, day] = date.split("-").map(Number);
  const [hour, minute] = time.split(":").map(Number);

  try {
    const response = await freeAstroFetch(new URL("/api/v2/vedic/panchang", API_BASE), {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: readableError(data) || `Panchang request failed with status ${response.status}.` },
        { status: response.status },
      );
    }
    if (data === null) {
      return NextResponse.json(
        { error: "FreeAstro Panchang returned an invalid response." },
        { status: 502 },
      );
    }
    ...
    return NextResponse.json(
      { error: networkError(error, "FreeAstro Panchang request failed.") },
      { status: 502 },
    );
  }
}

function parsePanchangInput(value: unknown): { input: PanchangInput } | { error: string } {
  if (!isRecord(value)) return { error: "Missing Panchang request payload." };
...
function networkError(error: unknown, fallback: string) {
  if (error instanceof Error && error.name === "TimeoutError") {
    return "FreeAstro Panchang request timed out.";
  }
  return fallback;
}
